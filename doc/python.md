...
2. 利用列表生成式可以遍历文件目录
3. 因为for循环可以使用2个变量和多个变量，因此列表生成式也可以
4. 使用列表生成式可以生成其他列表
5. 判断不为空时，推荐使用`is not None`

### 生成器

1. 列表生成式创建的列表是有限的，若仅仅需要访问后几个元素，其他元素占用的空间白白浪费了
2. 一边循环一边计算的机制，就叫生成器
3. 把一个列表生成式的`[]`换成`()`，就表示一个生成器
4. 生成器打印出来是内存地址
5. `next(generator)`获得生成器的下一个返回值
6. 生成器是可迭代对象，可以使用for
7. 函数定义中只要包含`yield`关键字，该函数就不是普通函数，而是一个生成器
8. 函数和生成器的执行流程不同，函数是遇到return就结束，生成器遇到`yield`就会返回，再次执行时从上次返回的`yield`语句继续执行
9. 为避免产生无限数列，要给for循环增加退出条件
10. 生成器有return语句时，必须捕获`StopIteration`才能获取返回值，返回值在`StopIteration`的value中

### 迭代器

1. 可以用for循环的对象统称为可迭代对象`Iterable`
2. 字符串也是可迭代对象
3. 可以被`next()`函数调用不断返回下一个值的对象称为`迭代器(Iterator)`，它表示一个惰性的计算序列
4. 生成器都是`Iterator`对象
5. `list`，`dict`，`str`虽然是`Iterable`，但不是`Iterator`
6. `iter()`函数把`Iterable`变成`Iterator`
7. `Iterator`对象表示的是数据流，无法提前知道序列的长度，只能通过`next()`函数计算下一个数，所以`Iterator`是惰性的，只有在需要返回下一个数时才会计算
8. for循环的本质就是不断调用`next()`函数实现的

## 函数式编程

1. 函数式编程允许把函数作为参数传入另一个参数，也允许返回一个函数
2. 纯粹的函数式编程没有变量
3. Python允许使用变量，所以Python不是纯函数式编程语言
4. 抽象程度低，越贴近计算机，执行效率高，抽象程度高，越贴近计算，执行效率低

### 高阶函数

1. 函数可以赋给变量，调用变量即是调用函数
2. 函数名也是变量，例如：`abs()`表示abs指向一个求绝对值的函数，可以变更abs的指向
3. 修改函数的指向只在当前模块生效，要在其他模块也生效，需要带上函数所在的模块名
4. 接收另一个函数作为参数的函数称为高级函数，传入函数名即可

#### map/reduce

1. `map()`函数接收两个参数，一个函数，一个`Iterable`，将传入的函数应用到每个序列上，并把结果作为新的`Iterator`返回
2. `list()`函数可以计算`Iterator`
3. `map()`作为高阶函数，把运算规则抽象了
4. 
